...
// pass jamerror by reference to all functions that can error in order to remove the nead to try-catch every errorable function call just to add more error information at each level.
...
// If it's performance critical and simple you can re-implement without this; or it's complex and string building is probably not that much of an overhead compared to the copmlex task.
class jamerror {
	private:
	...
	public:
	// reason is a machine readable version of the latest error message appended to message_stack.
	// This does not stack like message_stack for the same reasons message_stack is private, as described earlier.
	...
	enum reasons {
		reason_none = 0, // reason_none means no error occurred.
		reason_other = 1, // reason_other should be used when an error occured but you don't want to make the machine readable reason part of the external API. Mostly for implementation details that can change.
	...
	// Constructs an jamerror with message and reason. This state indicates that an error has occurred.
	jamerror(const std::string &message, int reason = reason_other);
	...
	// Prints the message_stack and returns reason int (handy for exiting program e.g. "int main() {jamerror err; return err.print();}").
	// message_stack is only accessible through printing so it cannot be accessed for the reasons stated above in the section about message_stack etc.
	int
	print() const;

	// Constructs an jamerror with message from errno in a thread safe way (using strerror_r instead of strerror).
	jamerror
	strerror();
};
